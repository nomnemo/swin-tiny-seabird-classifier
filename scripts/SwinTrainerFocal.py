...
def main():
    ...
    criterion = FocalLoss(alpha=focal_alpha, gamma=focal_gamma, reduction=focal_reduction)
    ...
    log(f"  loss:             FocalLoss")
    log(f"    alpha:          {focal_alpha}")
    log(f"    gamma:          {focal_gamma}")
    log(f"    reduction:      {focal_reduction}")
    log(f"  epochs:           {EPOCHS}")
